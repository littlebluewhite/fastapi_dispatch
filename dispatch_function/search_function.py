...
def deal_range(_range: list[str]):
    ...
    for item in _range:
        ...
        if len(r_list) != 3:
            raise DispatchException(status_code=406, detail="query range do not accept")
        if r_list[1] != "null":
...
def deal_value_in(_value_in: list[str]):
    ...
    for item in _value_in:
        ...
        if len(v_list) < 2:
            raise DispatchException(status_code=406, detail="query value in miss some params")
        if len(v_list) == 2:
...
def deal_json_in(_json_in: list[str]):
    ...
    for item in _json_in:
        ...
        if len(j_list) != 2:
            raise DispatchException(status_code=406, detail="query json in do not match")
